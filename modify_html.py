...
with open('attached_assets/Pasted--doctype-html-html-lang-en-head-meta-charset-UTF-8-meta_1772067532954.txt', 'r', encoding='utf-8') as f:
    content = f.read()

# Add CSS for WhatsApp float before </style>
whatsapp_css = """
...
"""
content = content.replace('</style>', whatsapp_css)

# Add Instagram to navlinks
nav_insta = """<nav class="navlinks">
...
content = re.sub(r'<nav class="navlinks">.*?</nav>', nav_insta, content, flags=re.DOTALL)

# Add Instagram and email to footer
footer_orig = """<footer>
...
# Add WhatsApp button before <script>
whatsapp_html = """
...
<script>
"""
content = content.replace('<script>', whatsapp_html)
